...
num_alt_vertices=$(cat chr${id}_snps_indel_POS_REF_ALT.txt | awk ' length($3)>1'| awk '{sum_1 = 0; t = split($3, arr, ","); for(i = 1; i <= t; i++) sum+=length(arr[i]);print sum}' | awk '{ sum += $1 } END { print sum }')
